# Lógica da API para cadastro e outras requisições de usuários
...
async def create_user(user: UserCreate, db: Session):
    ...
    user_db = await db.scalar(
        select(UserModel)
        .where(func.lower(UserModel.email) == user.email.lower())
    )
    if user_db:
...
async def read_logged_user(
    ...
    # não permitir acessar as infos de outros usuários
...
async def patch_user(
    ...
    # verifica se o novo email não causa conflito
    email_duplicado = None  # não é obrigatório passar email
