...
async def create_genero(
    genero: GeneroCreate, 
    db: Session, 
    current_user: CurrentUser  # usuário precisa estar logado
):
    # verificar se o gênero já existe
    genero_db = await db.scalar(
        select(GeneroModel)
        .where((func.lower(GeneroModel.genero)) == genero.genero.lower())
    )
...
async def read_generos(db: Session):
...
async def update_genero(
    ...
    genero_db = await db.scalar(
        select(GeneroModel)
        .where(GeneroModel.id == genero_id)
    )
    if not genero_db:
    ...
    genero_duplicado = await db.scalar(
        select(GeneroModel)
        .where(
            (GeneroModel.genero == genero.genero) 
            & (GeneroModel.id != genero_id)
        )
    )
    if genero_duplicado:
        raise HTTPException(status_code=HTTPStatus.CONFLICT, detail='Esse gênero já existe!')
